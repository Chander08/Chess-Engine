...
import logging
from chessbots import ChanderBot
...
class Game(threading.Thread):
    ...
    def handle_move(self, gameState):
        if not self.bots_turn(gameState):
            # Not our move so we can ignore but update gamestate to show the move I guess
            return
        bestMove = self.bot.getBestMove(gameState, self.playing_as)
